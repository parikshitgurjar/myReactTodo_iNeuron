...
function FormB() {
  ...
  const handleAddCard = (e) => {
    ...
    const newInProgressItem = { title, label, description };
    const updatedInProgressItems = [...inProgressItems, newInProgressItem];
    setInProgressItems(updatedInProgressItems);
    localStorage.setItem('inProgressItems', JSON.stringify(updatedInProgressItems));
  ...
  const handleRemoveCard = (index) => {
    const updatedInProgressItems = [...inProgressItems];
    updatedInProgressItems.splice(index, 1);
    setInProgressItems(updatedInProgressItems);
    localStorage.setItem('inProgressItems', JSON.stringify(updatedInProgressItems));
  };
  ...
  const handleUpdateCard = (index, updatedItem) => {
    const updatedInProgressItems = [...inProgressItems];
    updatedInProgressItems[index] = updatedItem;
    setInProgressItems(updatedInProgressItems);
    localStorage.setItem('inProgressItems', JSON.stringify(updatedInProgressItems));
  };

  useEffect(() => {
    const storedInProgressItems = JSON.parse(localStorage.getItem('inProgressItems')) || [];
    setInProgressItems(storedInProgressItems);
  }, []);
